 ...
 * Change Logs:
 * Date           Author       Notes
 * 2022-06-07     admin       the first version
 */
#include <rtthread.h>
#include <pin.h>
...
void Key_SemInit(void)
{
    ...
    K0_K1_Long_Sem = rt_sem_create("K0_K1_Long", 0, RT_IPC_FLAG_FIFO);
}
    ...
    if (K0_OnceFlag == 0)
    {
        if (K0_Long_Sem_Counter > 6)
        {
            if (K1_Long_Sem_Counter == 0)
            {
                ...
                LOG_D("K0 is PushLong\r\n");
            }
        }
        else
        {
            LOG_D("K0 PushLong Counter is%d",K0_Long_Sem_Counter);
        }
    }
}
    ...
    if (K1_OnceFlag == 0)
    {
        if (K1_Long_Sem_Counter > 6)
        {
            ...
            if (K0_Long_Sem_Counter > 5)
            {
                ...
                LOG_D("K0_K1 is PushLong \r\n");
            }
            else
            {
                rt_sem_release(K1_Long_Sem);
                LOG_D("K1 is PushLong \r\n");
            }
        }
        else
        {
            LOG_D("K1 PushLong is %d \r\n",K1_Long_Sem_Counter++);
        }
    }
}
...
void K1_LongFree_Release(void *parameter)
{
    ...
    LOG_D("K1 is PushLongFree\r\n");
}



void Detect_K0_K1(void)
{
    if (K0_Long_Sem_Counter > 5 && K1_Long_Sem_Counter > 5)
    {
        K0_OnceFlag = 1;
        K1_OnceFlag = 1;
...
void Button_Init(void)
{
    ...
    if (button_task != NULL) rt_thread_startup(button_task);
}
